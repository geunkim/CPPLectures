...
이러한 요구에 따라 객체 지향 프로그래밍 언어의 클래스(Class)라는 개념이 만들어 졌으며 C++ 언어에서도 클래스 기능을 지원하였다. 클래스가 제공하는 기능을 활용하면 
...
* private: private 접근 지정자는 갖는 멤버 변수, 멤버 함수는 클래스 외부에서는 접근이 불가능 것으로 클래스 내부의 멤버에 의해서만 
접근이 가능하다. 
```C++
#include <iostream>
