...
```20 - 11``` 연산은 보수를 이용한 덧셈으로 변경하면 (20 + 88) = 108 가 얻어진다.
여기서 두 자리 범위를 넘어간(오버플로우) 1 을 나머지 두 자리의 값(08) 에 더하여 ```9```의 값을 얻게되고 이 것이 
```20 - 11```의 결과이다. 

```20 - 30``` 연산은 보수를 이용한 덧셈으로 변경하면 (20 + 69) = 89 가 얻어진다. 
여기서 두 자리 범위를 넘어간 수(오버플로우)가 0이고 을 나머지 두 자리의 값인 89는 음수이다. 89의 9의 보수를 10이므로 
...
```20 - 11 ```은 ```00010100 + 11110100```로 표현되고 결과는 **1**00001000 로 오버플로우가 발생하여 최종 결과는 
```00001000 + 1```로 ```00001001``` 로 10진수로 9이다. 

```20 - 30```은 ```00010100 + 11100001``` 로 표현되고 결과는 111110101 로 오버플로우는 발생하지 않고 MSB가 ```1```로 음수이다. 
```111110101```의 1의 보수가 000001010로 10진수로 -10이다. 

#### 저장 공간의 크기와 표현이 가능한 정수 값 

8bit로 표현하면 1bit가 부호 비트로 사용된다. **0**0000000 은 0,  
양수는 **0**0000001 ~ **0**1111111 
