# 프렌즈 함수 (friend function) 와 프렌즈 클래스 (friend class)
...
프렌드 함수는 클래스의 멤버 함수가 아니기 떄문에 상속되지 않기 때문에 클래스 내의 어디에서든지 선언할 수 있다. 프렌즈 키워드와 선언할 수 있는 것은 **외부 함수**, **다른 클래스의 멤버 함수**, **다른 클래스** 세 종류이다.

## 프렌즈 함수가 왜 필요한가?
두 개의 객체를 연산하는 함수를 클래스의 멤버 함수로 작성하는 경우 클래스의 멤버 함수의 입력 파러미터를 같은 클래스의 객체를 전달하여야 한다. 
