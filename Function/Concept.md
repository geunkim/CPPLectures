...
* 함수 호출(function call): 정의된 함수의 기능을 실행하는 것으로, 실행을 위해 필요한 입력 데이터를 전달하고 함수가 기능을 수행하고 결과를 반환하는 경우
...
**srand** 함수는 시작 값 설정에 사용할 양수의 정수 값을 입력받아 난수를 계산하는 기능에 초기 값을 사용하여 난수를 생성하도록 설정만 할 뿐 반환 값은 없다. 
...
4. 입력데이터는 없고 반환 데이터는 있는 함수 예 

입력데이터는 없고 반환 데이터가 있는 함수 예로 난수(랜덤 숫자)를 생성하는 난수 생성 함수  ```rand``` 가 있다.
**rand()** 함수는 [0 ~ RAND_MAX] 범위의 정수 중 랜덤한 숫자 하나를 반환한다. RAND_MAX 값은 ```stdlib``` 헤더파일에 매크로로 다음과 같이 명시되어 있다.

```c++
#define RAND_MAX 0x7fff
```

난수 생성 함수 ```rand``` 함수의 원형은 다음과 같다.
```c++
int rand(void);
```

다음은 앞에서 살펴본 ```srand()```와 ```rand()```함수를 사용하여 난수를 생성하는 [프로그램](../SampleCodes/Function/randomNum.cc) 코드 이다.
...
함수의 기능을 실행시키는 것을 함수 호출(function call)이라고 한다. 함수를 호출하려면 함수의 이름을 적으며 함수가 필요로 하는 입력 데이터가 있으면 
데이터를 나열하면 된다. 함수 호출도 C++의 하나의 명령문이기 때문에 문장의 끝이 세미콜론(;)붙인다. 또한 함수가 반환 데이터가 있으면 반환데이터를 처리하는 기능이 필요하다. 

함수를 호출하게 되면 함수 호출문 다음의 코드의 실행은 잠시 중지되고 호출된 함수의 몸체 내의 문장들이 순차적으로 실행된 후 함수의 실행이 종료되면 함수 호출문 다음 위치로 되돌아 온다. 
...
알려주어야 할 함수의 정보를 함수 원형(function prototype)이라 한다. 함수 원형은 함수의 헤더와 유사한 형태를 가진다. 단 함수 선언에 사용되는 함수의 원형에는 매개 변수의 자료형이 의미를 가진다. 즉 함수 선언은 함수 원형을 코드로 작성하는 것이다. 
...
```
함수 선언은 컴파일러에게 미리 함수 원형에 대한 정보를 제공하여 함수 호출 시 함수의 반환형과 매개 변수의 자료형이 올바른 것인지 검사하도록 한다. 
컴파일러가 함수 호출 문장을 해석 및 검사할 때 컴파일러가 함수의 원형을 기반으로 함수 호출문이 올바른지를 검사할 수 있기 때문에 함수 원형을 알려 주는 함수 선언이 함수 호출보다 먼저 이루어져야 한다. 
함수의 정의가 함수의 호출보다 앞서 이루어져 있으면 컴파일러가 함수 호출문을 해석할 때 함수 정의를 해석할 때 얻은 사전 정보를 알고 있기 때문에 함수의 정의가 함수 선언의 의미를 동시에 갖게된다. 
...
* 프로그램을 함수라는 구조적인 프로그램으로 모듈화하기 때문에 프로그램을 수정할 때 모듈 별로 수정할 수 있어 프로그램의 유지 보수가 쉽다.
* 여러 프로그래머가 공동으로 참여하는 프로젝트에서 특정 프로그래머가 작성한 함수를 라이브러리화하여 프로젝트 내의 다른 개발자 들과 공유할 수 있다. 
